...
def main():
    if len(sys.argv) != 3:
        sys.exit('usage: {} <src_dir> <dst_dir>'.format(sys.argv[0]))
    ...
    dir_dst = os.path.normpath(sys.argv[2])
    ...
    for curr, image in enumerate(images):
        ...
        blend(image, images[next], dir_dst)
